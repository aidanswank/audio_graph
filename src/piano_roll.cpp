...
{
    smf::MidiFile& midi_file = midi_map[node_id];
    smf::MidiEventList& midi_track = midi_file[0];
...
//    sprintf_l("p %s", node_id)
    ...
    ImGui::Begin(piano_roll_string, is_open, ImGuiWindowFlags_HorizontalScrollbar);
...
//                    print("left",left_tick,"right",right_tick);
                    ...
                    if(left_tick<right_tick)
                    {
                        midi_track[note_right_drag_idx].getLinkedEvent()->tick = right_tick;
                    }
